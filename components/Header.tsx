      ...
      <header className="sticky top-0 z-50 bg-white border-b border-[#E6E6E6] print:hidden">
        <div className="max-w-7xl mx-auto px-4 h-[52px] flex items-center justify-between">
          ...
          <nav className="flex h-full items-stretch overflow-x-auto scrollbar-hide">
            ...
            {/* Show Messaging for Owner OR Visitor if they have messages */}
            {(currentUser === 'owner' || visitorHasMessages) && (
                 <Link to="/messages" className={navItemClass('/messages')}>
                  <div className="relative">
                    <MessageSquare className="w-5 h-5 md:w-6 md:h-6 mb-0.5" />
                    {currentUser === 'owner' && unreadTotal > 0 && (
            ...
            {currentUser === 'owner' ? (
              <>
                ...
                <button onClick={logout} className="flex flex-col items-center justify-center px-4 md:px-6 py-1 text-xs md:text-sm font-normal text-[#666666] hover:text-[#1A1A1A] transition-colors cursor-pointer">
                  <LogOut className="w-5 h-5 md:w-6 md:h-6 mb-0.5" />
